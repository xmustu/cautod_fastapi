...
class InternalGear(BaseGear):
    """
    Internal (ring) gear created by subtracting a matching external gear
    from a ring blank.
    """
    ...
    def __init__(
        ...
        alpha: float = 20,
        helix_angle: float = 0,
        backlash: float = 0.1,         # 额外齿侧间隙，防止过盈
        raw: bool = False,
        ...
        Args
        ----
        m, z, b, alpha, helix_angle, raw : same as Gear
        backlash : radial backlash (mm).  Typically 0.05~0.2*m
        """
        super().__init__(m, z, b, alpha, helix_angle)
        self.backlash = backlash
        self.raw = raw

        # 内齿轮的基准圆直径与外齿轮相同，但半径方向相反
        self.r_a = self.r_p + 1.25 * m   # 内齿轮齿顶圆 = 外齿轮齿根圆
        self.r_f = self.r_p - 1.00 * m   # 内齿轮齿根圆 = 外齿轮齿顶圆 + 间隙
        self.r_b = self.r_p * cos(self.alpha)

    # ----------------------------------------------------------
    def build(self):
        """
        Returns the internal gear as a cq.Workplane (solid)
        """
        # 1) 生成外齿轮（齿刀）—— 但半径方向需额外加上 backlash
        cutter = Gear(
            m=self.m,
            z=self.z,
            b=self.b,
        ...
        blank = (
            cq.Workplane("XY")
            .circle(self.r_a + 0.5*self.m)          # 毛坯外径略大
            # .circle(self.r_f - 1.5 * self.m) # 毛坯内径随意（只要 < r_f 即可）
            .extrude(self.b)
        )

        # 3) 把齿刀旋转到齿槽中心位置，再切除
        rot_angle = 180 / self.z          # 让齿刀齿与齿槽中心对齐
        if self.helix_angle != 0:
            # 对于斜齿，需要把角度绕 Z 轴旋转后再做螺旋匹配
            cutter = cutter.rotate((0, 0, 0), (0, 0, 1), rot_angle)
        else:
            cutter = cutter.rotate((0, 0, 0), (0, 0, 1), rot_angle)

        # 4) 布尔差得到内齿轮
        internal_gear = blank.cut(cutter)
